...
void limits_init() 
{
  // Configure limit pins as outputs and drive low by default per custom M-code control.
  // NOTE: This disables their use as limit switch inputs. Use with caution.
  LIMIT_DDR |= (LIMIT_MASK);   // Set as output pins
  LIMIT_PORT &= ~(LIMIT_MASK); // Drive outputs low (default state)

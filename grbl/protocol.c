    ...
    while((c = serial_read()) != SERIAL_NO_DATA) {
      if ((c == '\n') || (c == '\r')) { // End of line reached
        ...
        if (line_flags & LINE_FLAG_OVERFLOW) {
          ...
          // Intercept custom M-codes M11/M22/M33/M44 to control D4/D7/D9/D10 outputs directly.
          if (line[0] == 'M') {
            uint16_t m = 0;
            uint8_t i = 1;
            while (line[i] >= '0' && line[i] <= '9') { m = m*10 + (line[i]-'0'); i++; }
            if (m == 11 || m == 22 || m == 33 || m == 44) {
              int8_t s_val = -1;
              while (line[i] != 0) {
                if (line[i] == 'S') {
                  i++;
              ...
              if (m == 11) {
                // Forcefully repurpose D4 (overrides Z step/dir if present).
                DDRD |= (1<<4); // D4
                if (s_val == 0) { PORTD &= ~(1<<4); }
                else { PORTD |= (1<<4); }
              } else if (m == 22) {
                // Forcefully repurpose D7 (overrides Z step/dir if present).
                DDRD |= (1<<7); // D7
                if (s_val == 0) { PORTD &= ~(1<<7); }
                else { PORTD |= (1<<7); }
              } else if (m == 33) {
                // Temporarily repurpose X limit pin as output when requested.
                if (s_val == 0) {
                  // Turn off: drive low then restore as input and re-enable PC interrupt if needed.
                  LIMIT_PORT &= ~(1<<X_LIMIT_BIT);
                  LIMIT_DDR &= ~(1<<X_LIMIT_BIT);
                  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
                    LIMIT_PCMSK |= (1<<X_LIMIT_BIT);
                    PCICR |= (1 << LIMIT_INT);
                  }
                } else {
                  // Turn on: disable PC interrupt for this pin and set as output high.
                  LIMIT_PCMSK &= ~(1<<X_LIMIT_BIT);
                  LIMIT_DDR |= (1<<X_LIMIT_BIT);
                  LIMIT_PORT |= (1<<X_LIMIT_BIT);
                }
              } else {
                // Temporarily repurpose Y limit pin as output when requested.
                if (s_val == 0) {
                  LIMIT_PORT &= ~(1<<Y_LIMIT_BIT);
                  LIMIT_DDR &= ~(1<<Y_LIMIT_BIT);
                  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
                    LIMIT_PCMSK |= (1<<Y_LIMIT_BIT);
                    PCICR |= (1 << LIMIT_INT);
                  }
                } else {
                  LIMIT_PCMSK &= ~(1<<Y_LIMIT_BIT);
              ...
              // Not our custom M-code; fall through to normal parsing.
